    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
        ...
        // In your approve action section
        if ($_POST['action'] === 'approve') {
            $requestId = intval($_POST['request_id']);

            // Get the request details first
            $checkStmt = $conn->prepare("SELECT user_id, item_id FROM borrow_requests WHERE borrow_id = ?");
            ...
            // Proceed with approval if no duplicate exists
